...
This package combines data collected as part of ?????? conducted in Haiti. The projects were supported by SOIL at ??????.
...
The `mwater` data set has `r ncol(mwater)` variables and `r nrow(mwater)` observations. For an overview of the variable names, see the following table.
...
The `okap` data set has `r ncol(okap)` variables and `r nrow(okap)` observations. For an overview of the variable names, see the following table.
